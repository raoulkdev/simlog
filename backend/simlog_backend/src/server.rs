    ...
    pub async fn start(&self) -> Result<(), String> {
        match TcpListener::bind(&self.address).await {
            Ok(listener) => {
                println!(
                    "{}{}",
                    "Sucessfully started server at: ".bright_green(),
                    &self.address
                );
                Ok(axum::serve(listener, router::router(self.db.clone()))
                    .await
                    .map_err(|e| {
                        format!(
                            "{}{}, Error: {}",
                            "Error staring server at: ".bright_red(),
                            &self.address,
                            e
                        )
                    })?)
            ...
            Err(e) => Err(format!(
                "{}{}, Error: {}",
                "Error starting server at: ".bright_red(),
                &self.address,
                e
            )),
        }
